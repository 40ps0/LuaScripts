...
do -- Example UI
	local Window = library:AddWindow("Clicking Havoc", {
		main_color = Color3.fromRGB(41, 74, 122),
	...
	do -- Elements
		Tab2:AddLabel("KeyCode Toggle : RightShift")
        local xd = Tab2:AddDropdown("Credits", function(pd)
            setclipboard("https://github.com/zekgt/")
        end)
        xd:Add("UI : 0xSingularity")
        xd:Add("Script : ZekGT")
        xd:Add("UI Name : Elerium v2")
               
		local CP = Tab2:AddColorPicker(function(color)
			print(color)
		end)
		CP:Set(Color3.new(1, 0, 0))

        local Console = Tab2:AddConsole({
			["y"] = 100,
			["source"] = "Lua",
		})
		Console:Set("a = 2\nprint(a)")
        ...
        local HA = Tab2:AddHorizontalAlignment()
		HA:AddButton("Execute", function()
			loadstring(Console:Get())()
		end)

		local autoclick = Tab:AddSwitch("Autoclick", function(d)
            getgenv().apl = d;
            if not getgenv().apl then return end
			if getgenv().apl then
                while wait() do
        ...
        local wslr = Tab:AddSlider("WalkSpeed", function(x)
        ...
        local jmpslr = Tab:AddSlider("JumpPower", function(h)
